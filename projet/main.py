...
def affiche():
    ...
    if Game.menu == 2: ##affichage particulier du menu2
        ...
        if Game.choix == 1:
            ...
            co = (280,eguille2.topleft[1])
            screen.blit(im,co)
        elif Game.choix ==2:
            im = pygame.transform.rotate(eguille2.image,-20)
            co = (370,eguille2.topleft[1])
            screen.blit(im,co)
        elif Game.choix == 3:
            im = pygame.transform.rotate(eguille2.image,20)
            co = (430,eguille2.topleft[1])
            screen.blit(im,co)
        elif Game.choix == 4:
            im = pygame.transform.rotate(eguille2.image,80)
...
def jouer_son(valeur):
    # Initialiser Pygame
    pygame.mixer.init()

    # Construire le chemin du fichier wav
    chemin_fichier = os.path.join("assets", f"{valeur}.wav")

    # Vérifier si le fichier existe
    if not os.path.exists(chemin_fichier):
        print(f"Le fichier {valeur}.wav n'existe pas dans le répertoire 'assets'.")
        return
    ...
    try:
        ...
        # Le programme continue son exécution sans attendre la fin du son

    except pygame.error as e:
        print(f"Une erreur s'est produite lors de la lecture du son : {e}")
